		...
		<form action="{{ route('in.tipos_documento.actualizar', [$tipo_documento->id]) }}" method="POST">
			...
			<fieldset name="tipo-documento">
				<div class="group size-8 sangria required">
					...
					<input type="text" name="descripcion" id="descripcion" value="{{ $tipo_documento->descripcion }}" class="textarea size-9"  autocomplete="off">
				</div>
				<div class="group size-3 sangria required">
					<label>Estado: </label>
					<select id="estado" name="estado" class="input">
					    <option value="A" @if($tipo_documento->estado == 'A') selected @endif >Activo</option>
					    <option value="I" @if($tipo_documento->estado == 'I') selected @endif >Inactivo</option>
					</select>
				</div>
			</fieldset>
			<br>
