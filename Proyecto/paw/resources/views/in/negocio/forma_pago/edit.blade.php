		...
		<form action="{{ route('in.forma_pago.actualizar', [$formapago->id]) }}" method="POST">
			...
			<fieldset name="forma-pago">
				<div class="group size-8 sangria">
					...
					<input type="text" name="descripcion" id="descripcion" value="{{ $formapago->descripcion }}" class="textarea size-9"  autocomplete="off">
				</div>
				<div class="group size-3 sangria">
					<label>Estado: </label>
					<select id="estado" name="estado" class="input">
					    <option value="A" @if($formapago->estado == 'A') selected @endif >Activo</option>
					    <option value="I" @if($formapago->estado == 'I') selected @endif >Inactivo</option>
					</select>
				</div>
			</fieldset>
			<br>
